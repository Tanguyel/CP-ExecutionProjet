...
<?php
		...
		static function step_2() {
			...
			self::render( 'admin/view/steps/step-2', array(
                ...
                /*Ajout EP*/
                'course_objectives' => self::$settings['course_objectives'],
                'course_resources' => self::$settings['course_resources'],
                'course_public' => self::$settings['course_public'],
                'course_prerequisites' => self::$settings['course_prerequisites'],
                /*Fin Ajout EP*/
		...
		public static function step_6() {
            ...
            /*Ajout EP*/
            $base_formula_tag = self::$settings['base_formula_tag'];
            if ( empty( $base_formula_tag )) {
                $base_formula_tag = $is_paid_course ? 'paid': 'free';
            }
            $second_formula_tag = self::$settings['second_formula_tag'];
            $second_formula_tag = empty( $second_formula_tag ) ? 'free': $second_formula_tag;
			...
			self::render( 'admin/view/steps/step-6', array(
                ...
                /*Ajout EP*/
                'second_formula_course' => self::$settings['second_formula_course'],
                'base_formula_name' => self::$settings['base_formula_name'],
                'base_formula_tag' => $base_formula_tag,
                'second_formula_name' => self::$settings['second_formula_name'],
                'second_formula_tag' => $second_formula_tag,
                /*Fin Ajout EP*/
			) );
		}
		...
		public static function notice_about_pro_when_try_to_add_new_course() {
			echo '<p>';
			_e( 'The free version of CoursePress is limited to one course. To add more courses, upgrade to CoursePress for unlimited courses and more payment gateways.', 'coursepress' );
			echo '</p>';
			printf(
				'<p><a href="%s" class="button-primary">%s</a></p>',
				esc_url( __( 'https://premium.wpmudev.org/project/coursepress-pro/', 'coursepress' ) ),
				esc_html__( 'Try CoursePress for Free', 'coursepress' )
			);
		}
