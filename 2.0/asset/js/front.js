...
(function( $ ) {
	...
	CoursePress.Dialogs = {
			...
			if ( 0 === signup_errors.length && data['user_data']['logged_in'] === true ) {
				...
				if ( _coursepress.invitation_data ) {
					...
					$.each( steps, function( i, step ) {
						...
						if ( 'yes' === _coursepress.current_course_is_paid && 'paid_enrollment' === action && 'no' === _coursepress.current_course_has_second_formula) { //Ajout EP CP (3eme condition)
							CoursePress.Enrollment.dialog.openAt( i );
		...
		attempt_enroll: function( enroll_data ) {
			...
			if ( undefined === nonce || undefined === course_id ) {
                ...
                var group_tag_free = $(temp).attr('data-group-tag-free'); // Ajout EP CP 2 Formule
			}
			...
			var data = {
				...
				course_id: course_id,
				step: '',
                group: group_tag_free // Ajout EP CP 2 Formule
			};
	...
	CoursePress.EnrollStudent = function() {
		...
		// Is paid course?
		if ( 'yes' === _coursepress.current_course_is_paid && 'no' === _coursepress.current_course_has_second_formula) { //Ajout EP CP (2eme condition)
			$(newDiv).html(CoursePress.Enrollment.dialog.render().el);
