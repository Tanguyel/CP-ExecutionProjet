...
(function( $ ) {
	...
	CoursePress.Helpers.Module.refresh_ui = function() {
        ...
        /****Ajout EP CP : Modification de la fonction pour enlever le bind a use-timer
        CoursePress.Helpers.Module.form.bind_buttons();		
        ...
        */
        CoursePress.Helpers.Module.form.bind_buttons();		
		// Enable/disable duration or minimum grade
		$( '.module-assessable input' ).on( 'change', function() {
			var input = $( this ),
				is_checked = input.is( ':checked' ),
				target = $( input.data( 'target' ), input.parents( '.module-header' ).first() ),
				inputs = target.find( 'input' )
	...
	CoursePress.Helpers.Module.render_module = function( module, current_order ) {
        ...
        /*
        Ajout des autres modules de l'unité qui peuvent etre autocorrigés
        */
        
        data['moduletoautocorrect'] = _coursepress.modules_to_autocorrect;           
        data['unit'] = module.get( 'post_parent' );
        /*
	...
	CoursePress.Helpers.Module.render_components = function( module, data ) {
        ...
        /******************************************************************
        Ajout d'un nouveau control pour le choix du module à autocorriger
        ******************************************************************/
                    case 'autocor-select':
                        ...
                        answers = data['moduletoautocorrect'] ;

						//answers = answers.length > 0 ? CoursePress.utility.unserialize( answers ) : item['answers'];
						answers = answers.length > 0 ? answers : '';

						question_selected = module.get_meta( 'question_selected' );
                        question_selected = parseInt( question_selected );
                        
                        content += '<select ' + attr + '>';
                        
						$.each( answers, function( index, answer ) {
                            
                            if (data['unit'] === answer['post_parent'] ) {
							     // Legacy answers
							     if ( question_selected === answer['ID'] ) 
								    selected = 'selected';
                                else
                                    selected = '';
                                
                                content += '<option value="' + answer['ID'] +'"' + selected + '>' + answer['post_title'] +'</option>';
                            }
        ...
        /******************************************************************
        Fin de l'ajout d'un nouveau control pour le choix du module à autocorriger
        ******************************************************************/
