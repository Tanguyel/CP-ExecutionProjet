...
(function( $ ) {
	...
	function bind_buttons() {
		...
		$( '.cp-box-content .button.step.prev, .cp-box-content .button.step.next, .cp-box-content .button.step.update, .cp-box-content .button.step.finish' ).on( 'click', function( e ) {
			...
			if ( 2 <= step ) {
				//required_fields.push( 'course_description' ); Ajout EP CP 
			}
			if ( 4 <= step ) {
        ...
        //Ajout EP CP pour 2 formules
        
		// "Create a 2nd Formula" checkbox
		$( '[name="meta_second_formula_course"]' ).on( 'change', function() {
			if ( this.checked ) {
				$( this ).parents( '.cp-box-content.step-6' ).find( '.has2_formula_toggle' ).removeClass( 'hidden' );
			} else {
				$( this ).parents( '.cp-box-content.step-6' ).find( '.has2_formula_toggle' ).addClass( 'hidden' );
			}
		} );
        
        //empeche d'avoir des tag vide
        $( '[name="meta_second_formula_tag"]' ).on( 'change', function() {
            if ($( this ).val()=='') {
                $(this).val($(this).parents( '.cp-box-content.step-6' ).find( '[name="meta_second_formula_name"]' ).val());
            }
		} );
        
        $( '[name="meta_base_formula_tag"]' ).on( 'change', function() {
            if ($( this ).val()=='') {
                $(this).val($(this).parents( '.cp-box-content.step-6' ).find( '[name="meta_base_formula_name"]' ).val());
            }
		} );
