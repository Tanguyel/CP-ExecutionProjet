	...
	public static function course_enrollment() {
		...
		switch ( $action ) {
			...
			case 'enroll_student':
				...
				} else if ( ! empty( $student_id ) && ! empty( $course_id ) && true === CoursePress_Data_Course::enroll_student( $student_id, $course_id, '', $group ) ) { //Ajout EP CP (2 derniers parametres)
					$json_data['student_id'] = $student_id;
					$json_data['course_id'] = $course_id;
					$json_data['success'] = true;
                    $json_data['group'] = $group; //Ajout EP CP
				} else {
					$json_data['error_message'] = __( 'Could not enroll at this time.', 'cp' );
					$json_data['success'] = false;
                    $json_data['student_id'] = $student_id;//temp a suprimer
					$json_data['course_id'] = $course_id;//temp a suprimer
                    $json_data['group'] = $group; //temp a suprimer
				}
			...
			case 'enroll_with_passcode':
				...
				if ( $course_passcode != $passcode ) {
                    ...
                    // Fin Ajout EP CP
					CoursePress_Data_Course::enroll_student( $student_id, $course_id, '', $group ); //Ajout EP CP (2 derniers parametres)
					$json_data['success'] = true;
                    $json_data['group'] = $group; //Ajout EP CP
				}
				$success = isset( $json_data['success'] ) ? $json_data['success'] : false;
