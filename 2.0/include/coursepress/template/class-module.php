...
//Ajout EP CP
            if ( defined( 'WP_DEBUG_LOG' ) ) {
                $GLOBALS['wp_log']['CoursepressMod'][] = 'render_input_radio loop $module : ' . $module->ID;
                $GLOBALS['wp_log']['CoursepressMod'][] = 'render_input_radio loop reponses : ' . $response;
            };
//Fin Ajout EP CP
	...
	public static function render_input_form( $module, $attributes = false, $student_progress = false, $disabled = false ) {
		...
		foreach ( $attributes['questions'] as $qi => $question ) {
			...
			$label = sprintf( '<label for="%s" class="question" style="display:block;">%s</label>', $field_id, esc_html( $question['question'] ) ); //Ajout EP CP class="question"
    ...
    /************************************************
    Debut Ajout pour question autocorrection
    *************************************************/

    public static function render_input_autocor( $module, $attributes = false, $student_progress = false, $disabled = false ) {
       $attributes = false === $attributes ? self::attributes( $module->ID ) : $attributes;
        $student_progress = false === $student_progress ? self::$student_progress : $student_progress;
        $initial_module_id = $attributes['question_selected'];

        if ( defined( 'WP_DEBUG_LOG' ) ) {
            $GLOBALS['wp_log']['CoursepressMod'][] = '$initial_module_id: ' . $initial_module_id;
        };

        $module_to_autocorrect = get_post( $initial_module_id); 
        $module_to_autocorrect_attributes = self::attributes( $initial_module_id);

        if ( defined( 'WP_DEBUG_LOG' ) ) {
            $GLOBALS['wp_log']['CoursepressMod'][] = 'Recuperation de la Question à autocorriger : ' . $module_to_autocorrect->post_title;
        };

        $content = '';

        //Affichage de la question a autocorriger 

        $initalquestiontitle =  __( 'The initial question and your answer were : ', 'coursepress' );

        $content .= '<div class="autocor-inital-question">';
        $content .= sprintf('<h5 class="autocor-inital-question-title">%s</h5>', $initalquestiontitle );

        $content .= self::_wrap_content( $module_to_autocorrect->post_content );

        //Creation d'un display specifique 

        $response_initial = self::get_response( $module_to_autocorrect->ID, get_current_user_id() );
        $content .= '<ul class="cp-answers">';

        foreach ( $module_to_autocorrect_attributes['answers'] as $key => $answer ) {
            if ( 'input-checkbox' !== $module_to_autocorrect_attributes['module_type'] ) {
                $student_answer = $response_initial == $key || $response_initial === $answer;
            } else {
                $student_answer = is_array( $response_initial ) ? in_array( $key, $response_initial ) : $response_initial == $key;
            }
            if ( $student_answer ) {
                $answer = '<span class="chosen-answer chosen"></span>' . $answer;
            } else {
                $answer = '<span class="chosen-answer notchosen"></span>' . $answer;
            }
            $content .= sprintf( '<li>%s</li>', $answer );
        }
        $content .= '</ul>';
        ...
        /*Precedente methode qui permet de reaficher les information tel que c'est presenté quand la question est posé
        $method = 'render_' . str_replace( '-', '_', $module_to_autocorrect_attributes['module_type'] );
        $content .= call_user_func( array( __CLASS__, $method ), $module_to_autocorrect, $module_to_autocorrect_attributes, $student_progress );

        if ( defined( 'WP_DEBUG_LOG' ) ) {
            $GLOBALS['wp_log']['CoursepressMod'][] = 'Methode appelé pour recuperer la reponse initial : ' . $method;
            $GLOBALS['wp_log']['CoursepressMod'][] = 'Retour de la methode : ' . call_user_func( array( __CLASS__, $method ), $module_to_autocorrect, $module_to_autocorrect_attributes, $student_progress );
        };
*/
        $content .= '</div>';

        //Affichage des posibilité pour l'autocorrection. 

        $newanswertitle =  __( 'Indicate your new answer if you want to change it : ', 'coursepress' );

        $content .= '<div class="autocor-new-answer">';
        $content .= sprintf('<h5 class="autocor-new-answer-title">%s</h5>', $newanswertitle );


        if ( defined( 'WP_DEBUG_LOG' ) ) {
            $GLOBALS['wp_log']['CoursepressMod'][] = '$module_to_autocorrect_attributes[answers] : ' . implode(' ; ', $module_to_autocorrect_attributes['answers']);
        };

        if ( ! empty( $module_to_autocorrect_attributes['answers'] ) ) {
            ...
            switch ( $module_to_autocorrect_attributes['module_type'] ) {
                case 'input-radio':
                    $content .= '<ul class="quiz-question quiz-question-radio">';
                    foreach ( $module_to_autocorrect_attributes['answers'] as $key => $answer ) {
                        $checked = '' !== $response ? ' ' . checked( 1, '' != $response && (int) $response === $key, false ) : '';
    ...
    /************************************************
    Fin ajouté pour question autocorrection
    *************************************************/
