	...
	public static function change_order_status( $order_id, $old_status, $new_status ) {
		...
		foreach ( $items as $item ) {
			...
			if ( 'completed' === $new_status ) {
                ...
                $paid_tag = $item['paid_tag'];
                if ($paid_tag == '') {
                    $paid_tag = CoursePress_Data_Course::group_tag($course_id, 'paid');
                }
                $paid_tag .= "-complete";
                //echo "paidtag : " . $paid_tag;
                //Fin Ajout EP CP formule
				CoursePress_Data_Course::enroll_student( $user_id, $course_id, '', $paid_tag ); //Ajout EP CP 2 formule (2 derniers parametres)
			}
		}
	}
...
<?php
	...
	public static function show_course_message_woocommerce_order_details_after_order_table( $order ) {
		...
		if ( is_user_logged_in() /*&& 'wc-completed' == $order->post_status*/ ) {// Ajout EP CP (supression seconde condition)
	...
	public static function enroll_button( $content, $course_id, $user_id, $button_option ) {
        ...
        //Ajout EP CP 2 Formule
        /**
		 * renvoyer vers le pannier quand on est pas enregistrer aussi
		 */
		if ( 'signup' == $button_option) {
...
<?php   }
        ...
        //fin Ajout EP CP 2 formule ?>
        <button type="submit" class="single_add_to_cart_button button alt apply-button"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>  <?php // Ajout EP CP 2 Formule ajout de apply-button dans la class ?>
...
<?php
	 ...
	 * @param arrat $atts Configuration array.
	 */
	public static function add_to_cart_template( $atts ) { //A Voir et modifier
		/**
		 * if we do not use woo, then we should not use this function
		 */
		if ( ! self::$is_active ) {
	...
	public static function allow_student_to_enroll( $enroll_student, $student_id, $course_id ) {
        ...
        $allow_enroll = true;
        if (CoursePress_Data_Course::is_paid_course( $course_id ) && !CoursePress_Data_Course::has_second_formula( $course_id )) {
            $allow_enroll = false;
        }
        return $allow_enroll;
		//return ! CoursePress_Data_Course::is_paid_course( $course_id ); //remplacé par la ligne du dessus
        //Fin Ajout EP CP 
	}
		 ...
		 */
            ...
            switch ( $new_status ) {
                ...
                case 'on-hold':
                    ...
                    $paid_tag .= '-hold';
                    CoursePress_Data_Course::enroll_student( $student_id, $course_id, '', $paid_tag );
                    break;
