...
class CoursePress_Data_Shortcode_Course {
	...
	public static function init() {
        ...
        /******** Ajout Execution Projet ************/
        
		add_shortcode(
			'course_pdf_description',
			array( __CLASS__, 'course_pdf_description' )
		);
        
        add_shortcode(
			'course_objectives',
			array( __CLASS__, 'course_objectives' )
		);     
        
        add_shortcode(
			'course_resources',
			array( __CLASS__, 'course_resources' )
		); 
        add_shortcode(
			'course_public',
			array( __CLASS__, 'course_public' )
		); 
        
        add_shortcode(
			'course_prerequisites',
			array( __CLASS__, 'course_prerequisites' )
		); 
        
        /******** FIN Ajout Execution Projet ************/
	}
	...
	public static function course_dates( $atts ) {
		extract( shortcode_atts( array(
			...
			'label_delimeter' => ':',
			'no_date_text' => __( 'No End Date', 'coursepress' ),
			'alt_display_text' => __( 'Course begins: ', 'coursepress' ),
			'show_alt_display' => 'no',
			'class' => '',
		), $atts, 'course_dates' ) );
        ...
        /******** Ajout EP CP **********/
        /* Modification du code ci dessous pour afficher le debut du cours clairement quand le cours n'as pas de date de fin 
        ...
        */
		...
		if ( $show_alt_display && $open_ended ) {
			if ( ! empty( $alt_display_text ) ) {
    			$content .= '<' . esc_html( $label_tag ) . ' class="label">' . esc_html( $alt_display_text ) . esc_html( $label_delimeter ) . '</' . esc_html( $label_tag ) . '>';
            }
            $content .= ' ';
            $content .= str_replace( ' ', '&nbsp;', date_i18n( $date_format, CoursePress_Data_Course::strtotime( $start_date ) ) ) ;
		} else {
	...
	public static function course_cost( $atts ) {
		...
		extract( shortcode_atts( array(
            ...
            'label_formule' => __('Formule : ', 'coursepress'),
            'label_formule_tag' => 'strong',
        ...
        /*******************************
            Ajout EP CP pour 2 formule
        *******************************/
        $has_second_formula = cp_is_true( CoursePress_Data_Course::get_setting( $course_id, 'second_formula_course', false ) );
        $base_formula_tag = CoursePress_Data_Course::get_setting( $course_id, 'base_formula_tag', 'paid' );
        $select ='';
        
        if ($has_second_formula) {
            $base_formula_name = CoursePress_Data_Course::get_setting( $course_id, 'base_formula_name' );
            $second_formula_name = CoursePress_Data_Course::get_setting( $course_id, 'second_formula_name') ;
            $second_formula_tag = CoursePress_Data_Course::get_setting( $course_id, 'second_formula_tag', "free" ) ;
            
            $select .= '<div class="course-formula course-formula-' . $course_id . ' ' . $class . '">';
            $select .=  '<' . esc_html( $label_formule_tag ) . ' class="label">' . esc_html( $label_formule )  . '</' . esc_html( $label_formule_tag ) . '> ';
            $select .= '<select name="course_formula" class="course_formula" >';
            $select .=     '<option class="' . $base_formula_tag . '" value="paid" selected="selected">' . $base_formula_name . '</option>'; 
            $select .=     '<option class="' . $second_formula_tag . '"value="free" >' . $second_formula_name . '</option>';
            $select .= '</select>';
            $select .= '</div>';
        }
        
        /*******************************
        Fin Ajout EP CP pour 2 formule
        *******************************/
        
		if ( $is_paid ) {
        // Ajout EP CP pour 2 formule
            $content .= '<span class="formule paid_price ' . $base_formula_tag . '">';
			$content .= apply_filters( 'coursepress_shortcode_course_cost', '', $course_id );//ligne initial non modifié
            $content .= '</span>';
            if ($has_second_formula) {
                if ( $show_icon ) {
                    $content .= '<span class="formule product_price free_price ' . $second_formula_tag . '">' . $no_cost_text . '</span>';
                } else {
                    $content .= '<span class="formule free_price ' . $second_formula_tag . '">' . $no_cost_text . '</span>';
                }
            }
        //Fin Ajout EP CP pour 2 formule
		} else {
		...
		if ( ! empty( $content ) ) {
			...
			//$content = '<div class="course-cost course-cost-' . $course_id . ' ' . $class . '">'; //Ligne Modifié EP CP reprise au dessus. 
        ...
        // Ajout EP CP 2 formule
        //$content .= '<script src="http://tanguyel.freeboxos.fr/WP/wp-content/plugins/coursepress%20EP/2.0/asset/js/formule.js" ></script>';
        $content .= '<script src="' . CoursePress::$url . 'asset/js/formule.js" ></script>';
    ...
    ********************************************************/
     ...
     /**
	 * return the adress of the pdf descriptive file.
	 *
	 * @since EP 0.0.8
	 * @param  array $atts Shortcode attributes.
	 * @return string Shortcode output.
	...
	public static function course_pdf_description( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => CoursePress_Helper_Utility::the_course( true ),
		), $atts, 'course_pdf_description' ) );

		$course_id = (int) $course_id;
		if ( empty( $course_id ) ) { return ''; }

		$pdf_description = CoursePress_Data_Course::get_setting( $course_id, 'pdf_description', '' );
	...
	public static function course_objectives( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => CoursePress_Helper_Utility::the_course( true ),
            'label' => __( 'Objectives', 'coursepress' ),
			'class' => '',
		), $atts, 'course_objective' ) );
		...
		$objectives = CoursePress_Data_Course::get_setting( $course_id, 'course_objectives', '' );

		if ( ! empty( $objectives ) ) {
			$content = $title;
            $content .= '<div class="course_objectives course_objectives-' . $course_id . ' ' . $class . '">';
			$content .= $objectives;
			$content .= '</div>';
    ...
    /**
	 * Shows the course Educational resources.
	 *
	 * @since EP 0.0.4
	 * @param  array $atts Shortcode attributes.
	 * @return string Shortcode output.
    ...
    public static function course_resources( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => CoursePress_Helper_Utility::the_course( true ),
            'label' => __( 'Executive Resources', 'coursepress' ),
			'class' => '',
		), $atts, 'course_resources' ) );
		...
		$resources = CoursePress_Data_Course::get_setting( $course_id, 'course_resources', '' );

		if ( ! empty( $resources ) ) {
			$content = $title;
            $content .= '<div class="course_resources course_resources-' . $course_id . ' ' . $class . '">';
			$content .= $resources;
			$content .= '</div>';
    ...
    public static function course_public( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => CoursePress_Helper_Utility::the_course( true ),
    ...
    public static function course_prerequisites( $atts ) {
		extract( shortcode_atts( array(
			'course_id' => CoursePress_Helper_Utility::the_course( true ),
