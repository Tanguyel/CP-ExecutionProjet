...
class CoursePress_Data_Shortcode_CourseTemplate {
	...
	public static function course_join_button( $atts ) {
		...
		extract( shortcode_atts( array(
			...
			'list_page' => false,
			'not_started_text' => __( 'Start on ', 'coursepress' ), //Ajout EP CP Changé Not Started par Start On
        ...
        /******* Ajout EP CP *********/
        $not_started_text = $not_started_text . str_replace( ' ', '&nbsp;', date_i18n( get_option( 'date_format' ), CoursePress_Data_Course::strtotime( $course->course_start_date ) ) );
        ...
        if ('enroll' == $button_option || 'signup' == $button_option) {
            $course->has_second_formula = cp_is_true( CoursePress_Data_Course::get_setting( $course_id, 'second_formula_course' ) );
            if (true == $course->is_paid && $course->has_second_formula) {
                $button2 = $button;
            }
        }
        ...
        //Warp in div in order to hide it easealy
        
        if ('enroll' == $button_option || 'signup' == $button_option) {
            $button = '<div class="enrollment-button '. $button_option . ' formule paid" >' . $button . '</div>';
            $button2 = '<div class="enrollment-button '. $button_option . ' formule free" >' . $button2 . '</div>';
        }
	...
	public static function course_structure( $atts ) {
		...
		extract( shortcode_atts( array(
			...
			'class' => '',
			'deep' => false,
            'full_structure' => false, //Ajout EP CP
        ...
        $full_structure = cp_is_true( sanitize_text_field( $full_structure ) ); //Ajout EP CP
		...
		foreach ( $units as $unit_id => $unit ) {
			...
			foreach ( $unit['pages'] as $key => $page ) {
				...
				if ( ! $full_structure && false === $enrolled && false === $can_update_course ) {
					if (
						! isset( $preview['structure'][ $unit_id ] )
						|| ! is_array( $preview['structure'][ $unit_id ] )
				...
				$page_title = $enrolled ? '<a href="' . esc_url( $page_link ) . '">' . esc_html( $page_title ) . '</a>' : sprintf( '<span>%s</span>', strip_tags( esc_html( $page_title ))); //Ajout EP CP ajout de l'enrobage span
				...
				if ( $enrolled && ! $clickable && ! $can_update_course && ! $full_structure) {
					continue;
				}
				...
				if ( $deep || 'section' === $structure_level || 'unit' === $structure_level ) {
					...
					foreach ( $page['modules'] as $m_key => $module ) {
                        ...
                        /*Ajout EP CP 
                        Recuperation du module seen pour l'ajouter aux class de la balise li
                        */ 
                        ...
                        $module_active = CoursePress_Helper_Utility::get_array_val( $student_progress, 'completion/' . $unit_id . '/last_module_seen/' );
                        $active_class = isset( $module_active ) && $module_active === $m_key ? 'module-active' : '';
						...
						$classes = array(
							'module',
							sprintf( 'module-%d ' . $seen_class . ' ' . $module_active , $module->ID ), //Ajout EP CP de $seen_class et $active_class
						);
						...
						//$module_seen = CoursePress_Helper_Utility::get_array_val( $student_progress, 'completion/' . $unit_id . '/modules_seen/' . $m_key ); Ajouté plus haut dans les modifs EP CP
						...
						//$seen_class = isset( $module_seen ) && ! empty( $module_seen ) ? 'module-seen' : ''; Ajouté plus haut dans les modifs EP CP
