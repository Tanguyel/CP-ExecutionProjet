...
class CoursePress_Data_Student {
	...
	public static function visited_module( $student_id, $course_id, $unit_id, $module_id, &$data = false ) {
		...
		/**************    Ajout EP CP    *************/
		$data = CoursePress_Helper_Utility::set_array_value( $data, 'completion/' . $unit_id . '/last_module_seen/', $module_id );
		/**************   Fin ajout EP CP *************/
		self::update_completion_data( $student_id, $course_id, $data );
		return $data;
	}
...
//Ajout EP CP
		if ( defined( 'WP_DEBUG_LOG' ) ) {
			$GLOBALS['wp_log']['CoursepressMod'][] = 'Execution de module reponse';
		};
//fin Ajout EP CP 
		if ( ! $refresh ) {
			/**
        ...
        /**************************
        Ajout EP CP pour module autocorect
        ****************************/
            case 'input-autocor':
                if ( defined( 'WP_DEBUG_LOG' ) ) {
                    $GLOBALS['wp_log']['CoursepressMod'][] = 'Execution de module reponse pour le cas input-autocor';
                };
                $module_to_autocorrect = get_post( $attributes['question_selected']); 
                $module_to_autocorrect_attributes = CoursePress_Data_Module::attributes( $attributes['question_selected']);
                $initial_response = self::get_response( $student_id, $course_id, $unit_id, $attributes['question_selected']);
                $initial_response = $initial_response['response'];
                //$bonne_reponse = (is_array($module_to_autocorrect_attributes['answers_selected'])? implode(' ; ', $module_to_autocorrect_attributes['answers_selected']) : $module_to_autocorrect_attributes['answers_selected'];

                if ( defined( 'WP_DEBUG_LOG' ) ) {
                    $GLOBALS['wp_log']['CoursepressMod'][] = 'Module to autocorect ID : ' . $module_to_autocorrect->ID;
                    $GLOBALS['wp_log']['CoursepressMod'][] = 'Type du module initial : ' . $module_to_autocorrect_attributes['module_type'];
                    $GLOBALS['wp_log']['CoursepressMod'][] = 'Bonne reponses array : ' . implode(' ; ', $module_to_autocorrect_attributes['answers_selected']);
                    $GLOBALS['wp_log']['CoursepressMod'][] = 'Bonne reponses string: ' . $module_to_autocorrect_attributes['answers_selected'];
                };
                switch ( $module_to_autocorrect_attributes['module_type'] ) {
                    case 'input-select':
                    case 'input-radio':
                        $answers_selected = $module_to_autocorrect_attributes['answers_selected'];
                        
                        if ( defined( 'WP_DEBUG_LOG' ) ) {
                            $GLOBALS['wp_log']['CoursepressMod'][] = 'Cas Select - Radio / $module_to_autocorrect_attributes [checked_answer] : ' . implode(' ; ', $module_to_autocorrect_attributes['checked_answer']) ;
                        }
                        
                        // Double check answer
                        if ( isset( $module_to_autocorrect_attributes['checked_answer'] ) ) {
                            $answers = $module_to_autocorrect_attributes['answers'];
                            foreach ( $answers as $k => $v ) {
                                if ( $answers_selected === $v ) {
                                    $answers_selected = $k;
                                }
                                if ( defined( 'WP_DEBUG_LOG' ) ) {
                                    $GLOBALS['wp_log']['CoursepressMod'][] = 'Cas Select - Radio / boucle answer selected : ' . $answers_selected . ' ; $k' . $k . ' ; $v' . $v ;
                        }
                            }
                        }

                        if ( defined( 'WP_DEBUG_LOG' ) ) {
                            $GLOBALS['wp_log']['CoursepressMod'][] = 'Cas Select - Radio / Reponse choisit : ' . (int) $response . ' ; ' . implode($response) . ' Bonne reponse : ' . $answers_selected . ' Initial reponse : ' . ($initial_response)  ;
                        }
                        
                        //if ( $response === $answers_selected ) {
                        if ( (int) $response == (int) $answers_selected ) {
                            $grade = ( (int) $response == (int) $initial_response ) ? 100 : 90;
                            $autocor_case = ( (int) $response == (int) $initial_response ) ? 'BB' : 'MB';
                        } else {
                            $grade = 0;
                            $autocor_case = ( (int) $response == (int) $initial_response ) ? 'BM' : 'MM';
                        }
                        break;
                    case 'input-checkbox':
                        $selected = $module_to_autocorrect_attributes['answers_selected'];
                        $answers = $module_to_autocorrect_attributes['answers'];
                        $total = count( $selected );
                        $total_answers = count( $answers );
                        $ratio = $total_answers > 0 ? 100 / $total_answers : 0;
                        ...
                        if ( defined( 'WP_DEBUG_LOG' ) ) {
                            $GLOBALS['wp_log']['CoursepressMod'][] = 'Cas Checkbox / Reponse choisit : ' . implode(' ; ', $response);
                        }
                        ...
                        if ( is_array( $initial_response ) ) {
                            foreach ( $initial_response as $answer ) {
                                if ( in_array( $answer, $selected ) ) {
                                    $initial_correct++;
                                } else {
                                    $initial_wrong++;
                        ...
                        if ( 0 > $initial_correct ) {
                            $initial_correct = 0;
                        }
                        $initial_grade = 0;

                        if ( $initial_correct > 0 && $total > 0 ) {
                            $initial_grade = (int) $initial_correct * $correct_ratio;
                            

                            if ( $wrong > 0 ) {
                                $initial_grade -= $ratio * $initial_wrong;
                            }
                        }
                        if ( 100 == $grade )  {
                            $autocor_case = ( 100 == $initial_grade ) ? 'BB' : 'MB';
                        } else {
                            $autocor_case = ( 100 == $initial_grade ) ? 'BM' : 'MM';
                        }

                        break;
                }
                if ( defined( 'WP_DEBUG_LOG' ) ) {
                    $GLOBALS['wp_log']['CoursepressMod'][] = 'Cas Select - Radio / note et cas : ' . $grade . $autocor_case;
                }
                break;
		// Fin d'ajout EP CP
		}
