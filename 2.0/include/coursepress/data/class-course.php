...
class CoursePress_Data_Course {
	...
	public static function get_setting( $course_id, $key = true, $default = null ) {
		...
		$defaults = array(
            ...
            //Ajout EP 2 formule
            'base_formula_name' => __( 'Paid', 'coursepress' ),
            'base_formula_tag' => '',
            'second_formula_course' => false,
            'second_formula_name' => __( 'Free', 'coursepress' ),
            'second_formula_tag' => 'free',
            //fin Ajout EP 2 formule
			'enrollment_passcode' => '',
    ...
    //Donne vrai si il existe une seconde formule pour le cours
    
    public static function has_second_formula( $course_id ) {
		if ( empty( $course_id ) ) {
			return false;
		}
		/**
		 * check Course settings
		 */
		$has_second_formula = self::get_setting( $course_id, 'second_formula_course', false );
		$has_second_formula = cp_is_true( $has_second_formula );
		
		return $has_second_formula;
	}
    
    // donne le tag pour le group du cours par defaut celui gratuit
    
    public static function group_tag( $course_id, $type = 'free' ) {
		if ( empty( $course_id ) ) {
			return false;
		}
		/**
		 ...
		 */
        if ($type == 'free') {
            $group_tag = self::get_setting( $course_id, 'second_formula_tag', false );
        } else if ($type == 'paid'){
            $group_tag = self::get_setting( $course_id, 'base_formula_tag', false );
        } else {
            $group_tag = '';
        }
    ...
    // Change le groupe de l'etudiant dans le cours
    
    public static function change_group( $new_group, $course_id, $student_id ) {
		if ( empty( $course_id ) ) {
			return false;
		}
	...
	public static function enroll_student( $student_id, $course_id, $class = '', $group = '' ) {
		...
		if ( ! empty( $enrolled ) ) {
			//return $course_id;
            // Ajout EP CP 2 Formule
            $existing_group = get_user_option( $student_id, 'enrolled_course_group_' . $course_id ); 
			...
			if ( ! empty( $unit_id ) ) {
				...
				if ( ! $is_unit_available ) {
					...
					if ( ! empty( $unit_availability_date ) ) {
						$error_message = sprintf( __( 'This unit will be available on %s', 'coursepress' ), $unit_availability_date ); // Corection Bug EP CP; 
					} else {
